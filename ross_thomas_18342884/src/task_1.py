...
for img_file in img_files:
    ...
    write_image_to_work(args, img_file, "2_0", draw_regions(regions))

    regions = remove_overlapping(regions, threshold=0.8)
    write_image_to_work(args, img_file, "2_1", draw_regions(regions))

    regions = list(filter(lambda r: r.box.aspect() >= 0.8, regions))
    write_image_to_work(args, img_file, "2_2", draw_regions(regions))
