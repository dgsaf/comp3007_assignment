...
artifacts/blurring) but are closely adjacent to each other.
...
In summary, the following properties of the digits are almost always observed,
providing a robust foundation for detection of the digits:
\begin{itemize}
\item
  The uniformly white colour of the digits;

\item
  The uniformly black background of the sign;

\item
  The strong contrast between the white digits and the black background across
  all red, green, and blue channels as well as in grayscale;

\item
  The regular uniform height of the digits;

\item
  The regular spacing of the digits horizontally;
...
\begin{itemize}
\item
  The presence of strong radial, or motion blurring which may cause character
  regions to overlap;

\item
  The presence of strong shadowns, glare, or other variations in lighting
  conditions across the character regions;

\item
  The presence of a foreign object, such as a sticker or a mark, on the sign
  (violating the assumptions on regularity of layout) or across the character
  regions (violating the assumptions on uniformity of character regions).
